        ...
        <div
            ...
            <form action="{{ route('quality_control.update', $qualityControls) }}" method="POST"
                ...
                <div style="display: flex; flex-direction: column; gap: 5px; margin-top: 30px;">
                    <select id="jenis_industri" name="jenis_industri" required style="height: 35px;">
                        ...
                        @foreach($jenis_industri as $jenis)
                            <option value="{{ $jenis }}" {{ old('jenis_industri', $qualityControls->jenis_industri) == $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                        @endforeach
                    </select>
                    ...
                    <select id="nama_perusahaan" name="nama_perusahaan" required style="height: 35px;">
                        ...
                        @foreach($nama_perusahaan as $nama)
                            <option value="{{ $nama }}" {{ old('nama_perusahaan', $qualityControls->nama_perusahaan) == $nama ? 'selected' : '' }}>{{ $nama }}</option>
                        @endforeach
                    </select>
                    <script>
                ...
                <div style="display: flex; flex-direction: column; gap: 5px;">
                    ...
                    <label for="supporting_recommendation" style="font-weight: bold; color: #555;">Rekomendasi
                        Pendukung</label>
