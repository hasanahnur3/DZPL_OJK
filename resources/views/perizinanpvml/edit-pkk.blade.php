...
<div style="max-width: 800px; margin: auto; padding: 2rem;  border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
    ...
    <form method="POST" action="{{ route('kepengurusan.update', $penilaian->id) }}"
        ...
        <div style="display: flex; flex-direction: column; gap:5px;">
            ...
            <select name="jenis_industri" required
                ...
                <option value="Pergadaian" {{ $penilaian->jenis_industri == 'Pergadaian' ? 'selected' : '' }}>Pergadaian
                </option>
            </select>
            <!-- Input Nama Perusahaan -->
            <label for="nama_perusahaan" style="font-weight: bold; color: #555;">Nama Perusahaan</label>
            <input type="text" name="nama_perusahaan" required value="{{ $penilaian->nama_perusahaan }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">

            <!-- Input Nama Pihak Utama -->
            <label for="nama_pihak_utama" style="font-weight: bold; color: #555;">Nama Pihak Utama</label>
            <input type="text" name="nama_pihak_utama" required value="{{ $penilaian->nama_pihak_utama }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">

            <!-- Input Jabatan -->
            <label for="jabatan" style="font-weight: bold; color: #555;">Jabatan</label>
            <input type="text" name="jabatan" required value="{{ $penilaian->jabatan }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">

            <!-- Input Status -->
            <label for="status" style="font-weight: bold; color: #555;">Status</label>
            <input type="text" name="status" required value="{{ $penilaian->status }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
            ...
            <input type="text" name="nomor_surat_permohonan" required value="{{ $penilaian->nomor_surat_permohonan }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
            ...
            <input type="date" name="tanggal_surat_permohonan" required
                value="{{ $penilaian->tanggal_surat_permohonan }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
            ...
            <input type="date" name="tanggal_pengajuan_sistem" required
                value="{{ $penilaian->tanggal_pengajuan_sistem }}"
                style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
        </div>
