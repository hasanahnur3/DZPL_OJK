            ...
            <form action="{{ route('tka.update', $tka->id) }}" method="POST"
                ...
                <div style="display: flex; flex-direction: column; gap: 5px; margin-top: 25px;">
                    <select id="jenis_industri" name="jenis_industri" required style="height:35px">
                        ...
                        @foreach($jenis_industri as $jenis)
                            <option value="{{ $jenis }}" {{ old('jenis_industri', $tka->jenis_industri) == $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                        @endforeach
                    </select>

                    <select id="nama_perusahaan" name="nama_perusahaan" required style="height:35px">
                        @foreach($nama_perusahaan as $nama)
                            <option value="{{ $nama }}" {{ old('nama_perusahaan', $tka->nama_perusahaan) == $nama ? 'selected' : '' }}>{{ $nama }}</option>
                        @endforeach
                    </select>
                    <script>
                    ...
                    <input type="text" name="nomor_surat_permohonan" id="nomor_surat_permohonan"
                        value="{{ $tka->nomor_surat_permohonan }}" required
                    ...
                    <label for="tanggal_surat_permohonan" style="font-weight: bold; color: #555;">Tanggal Surat
                        Permohonan</label>
                    <input type="date" name="tanggal_surat_permohonan" id="tanggal_surat_permohonan"
                        value="{{ $tka->tanggal_surat_permohonan }}" required
                        style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
                    ...
                    <select name="status_perizinan" id="status_perizinan" required
                        style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
                        <option value="Dalam proses analisis" {{ $tka->status_perizinan == 'Dalam proses analisis' ? 'selected' : '' }}>Dalam proses analisa</option>
                        <option value="Kelengkapan dok" {{ $tka->status_perizinan == 'Kelengkapan dok' ? 'selected' : '' }}>
                ...
                <div style="display: flex; flex-direction: column; gap: 5px;">
                    ...
                    <select name="jenis_output" id="jenis_output" required
                        style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
                        <option value="pencatatan" {{ $tka->jenis_output == 'pencatatan' ? 'selected' : '' }}>Surat Pencatatan
                        </option>
                        <option value="penolakan" {{ $tka->jenis_output == 'penolakan' ? 'selected' : '' }}>Surat Penolakan</option>
                    </select>
                    ...
                    <input type="text" name="no_surat_pencatatan" id="no_surat_pencatatan"
                        value="{{ $tka->no_surat_pencatatan }}" required
                        style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
                    ...
                    <input type="date" name="tanggal_surat_pencatatan" id="tanggal_surat_pencatatan"
                        value="{{ $tka->tanggal_surat_pencatatan }}"
                        style="padding: 0.75rem; border: 1px solid #ccc; border-radius: 10px;">
